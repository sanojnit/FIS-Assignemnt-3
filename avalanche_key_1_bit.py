...
for j in range(1,65):
    for i in range(1,65):
        ...
        c = n >> (N - dd)
        key = b | c
        print("New Key Generated :",bin(key))
        print("New Key Generated :",bin(key_old))
        no_bits_changed=bin(key).count("1")-bin(key_old).count("1")
...
for j in range(1,65):
    for i in range(1,65):
        ...
        print("generated key",bin(temp_key),len(bin(temp_key)),"count",i)
        print("Old Key",bin(key_initial),len(bin(key_initial)))
        #insert
        if j==1:
            new_initial=key_initial
        print("------------------")
        print("Plain Text :",plaintext)
        print("Plain Text in binary :",plaintext_bits)
        ...
        #kdiff=temp_key.count("1")-key_initial.count("1")
        print("lengths",)
        kdiff=bin(temp_key).count("1")-bin(new_initial).count("1")
        ...
        if i==64:
            ...
            print("Initial Key",bin(key_initial),len(bin(key_initial)))
            new_initial=temp_key
            encrypted_text1=encrypted_text2
...
with pd.ExcelWriter('ONE_KEY.xlsx') as writer:
